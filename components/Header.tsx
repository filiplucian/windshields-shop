      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 md:h-18">
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <a href={`https://wa.me/${SITE_CONFIG.phonesRaw[0].replace('+', '')}`} target="_blank" rel="noopener noreferrer" className="btn-whatsapp text-xs px-4 py-2.5">
              <WhatsAppIcon className="w-4 h-4" />
              WhatsApp
            </a>
            <a href={`tel:${SITE_CONFIG.phonesRaw[0]}`} className="btn-primary text-xs px-4 py-2.5">
              Sună Acum
            </a>
          </div>
          ...
          <div className="flex lg:hidden items-center gap-2">
            <a href={`tel:${SITE_CONFIG.phonesRaw[0]}`} className="flex items-center justify-center w-10 h-10 rounded-full bg-brand-red text-white" aria-label="Sună">
          ...
          <nav className="lg:hidden py-4 border-t border-white/10 flex flex-col gap-3">
            {navLinks.map((link) => (
              <a
                ...
                className="font-heading font-semibold text-brand-mid-gray uppercase tracking-wide hover:text-white transition-colors py-1"
              >
                {link.label}
              </a>
            ))}
            <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 w-full py-2.5 rounded bg-[#1877F2] text-white font-heading font-semibold text-sm hover:bg-[#166FE5] transition-colors touch-manipulation"
              >
                <FacebookIcon className="w-5 h-5" />
                Facebook – Centru de Parbrize
              </a>
              <a href={`https://wa.me/${SITE_CONFIG.phonesRaw[0].replace('+', '')}`} target="_blank" rel="noopener noreferrer" className="btn-whatsapp w-full">
                <WhatsAppIcon className="w-5 h-5" />
                WhatsApp – {SITE_CONFIG.phones[0]}
              </a>
              <a href={`https://wa.me/${SITE_CONFIG.phonesRaw[1].replace('+', '')}`} target="_blank" rel="noopener noreferrer" className="btn-whatsapp w-full">
                <WhatsAppIcon className="w-5 h-5" />
                WhatsApp – {SITE_CONFIG.phones[1]}
              </a>
              <a href={`tel:${SITE_CONFIG.phonesRaw[0]}`} className="btn-primary w-full">
                Sună: {SITE_CONFIG.phones[0]}
              </a>
            </div>
          </nav>
        )}
